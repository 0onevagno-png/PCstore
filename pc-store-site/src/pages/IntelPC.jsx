...
const IntelPC = () => {
  ...
  const filteredBuilds = intelBuilds.filter((build) => {
    return (
      (!filters.gpu ||
  ...
  const extractOptions = (field) => {
    const options = intelBuilds.map((b) => b.specs[field]?.name || "");
  ...
  const handleCardClick = (id) => {
    navigate(`/pc/intel/${id}`);
  };

  return (
    <div className="bg-black min-h-screen text-white">
      <div className="px-8 py-12">
        <h1 className="text-4xl font-bold mb-6">Игровые ПК Intel</h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedBuilds.map((build, idx) => (
            <div
              key={idx}
              onClick={() => handleCardClick(build.id || idx)}
              className="cursor-pointer block bg-[#151515] rounded-xl p-6 border border-[#333] shadow-md hover:scale-105 transition hover:ring-1 hover:ring-yellow-500">
