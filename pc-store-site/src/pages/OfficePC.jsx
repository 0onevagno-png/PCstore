...
const OfficePC = () => {
  ...
  const filteredBuilds = officeBuilds.filter((build) => {
    return (
      (!filters.gpu ||
  ...
  const extractOptions = (field) => {
    const options = officeBuilds.map((b) => b.specs[field]?.name || "");
  ...
  return (
    <div className="bg-black min-h-screen text-white">
      <div className="px-8 py-12">
        <h1 className="text-4xl font-bold mb-6">Офисные ПК</h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedBuilds.map((build, idx) => (
            <div
              key={idx}
              onClick={() => navigate(`/pc/office/${build.id || idx}`)}
